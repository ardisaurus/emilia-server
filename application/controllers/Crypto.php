...
class Crypto extends REST_Controller {
    ...
    function index_get() {
        ...
        if ($plain && $session_id && $this->session_id_check($session_id)==true) {
            $device = $this->db->query("SELECT `public_key`, `modulo` FROM `session` where `session`.`session_id`='$session_id'")->result();
            $cipher = $this->rsa_encrypt($plain,  $device[0]->public_key,  $device[0]->modulo);
            $data[0] = array('cipher' => $cipher);
