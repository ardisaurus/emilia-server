...
class Device extends REST_Controller {
    ...
    function index_post() {
        ...
        if($action=="check") {
            ...
            $plain=$this->dec($session_id, $cipher_rsa, $cipher_sym);
            $plainfactor=$this->factors($plain, true);
            if (isset($device[0]->dvc_status)) {
                if ($device[0]->dvc_status=="1") {
                    $data = array('status_code' => $plainfactor[0]);
                    $this->response(array("result"=>$data, 200));
                }else{
                    $data = array('status_code' => $plainfactor[1]);
                    $this->response(array("result"=>$data, 200));                    
                }
            } else {
    ...
    function dec($session_id, $cipher_rsa, $cipher_sym){        
